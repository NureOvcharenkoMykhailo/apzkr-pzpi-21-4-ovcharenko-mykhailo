...
<main class="dark:text-white flex-col justify-stretch w-fit m-auto gap-5">
	<h5 class="mb-4 font-bold">{$translate("food.new")}</h5>
	...
	<Input
		type="number"
		id="carbs"
		placeholder={$translate("carbs", { v: "" })}
	/>
	<Input
		type="number"
		id="protein"
		placeholder={$translate("protein", { v: "" })}
	/>
	<Input type="number" id="fat" placeholder={$translate("fat", { v: "" })} />
	<Input
		type="number"
		id="calories"
		placeholder={$translate("calories", { v: "" })}
	/>
	...
	<Button
		on:click={async () => {
			...
			let response = await POST("/food/create", post_data, [
